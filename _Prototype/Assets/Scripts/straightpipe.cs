...
public class straightpipe : MonoBehaviour
{
    ...
    private void OnTriggerStay(Collider other)            //while the sphere is in the pipe
    {
        ...
        if (Mathf.Min(distance) < 0.1f)     //makes sure the sphere is close to the waypoints
        {
            ...
            if (Mathf.Min(distance) == distance[2])          //if the sphere is at the other end of the pipe
            {
        ...
        {
            Destroy(other.gameObject);             //otherwise it is not entering the pipe at the right ends so it is destroyed since it is very far away from the waypoints
        }
    }
